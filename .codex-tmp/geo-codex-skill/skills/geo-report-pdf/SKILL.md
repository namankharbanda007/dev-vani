...
- The PDF generation script is located at: `scripts/generate_pdf_report.py` within the installed `geo` skill bundle.
...
```bash
python scripts/generate_pdf_report.py /tmp/geo-audit-data.json GEO-REPORT-[brand].pdf
```
...
6. **Run the PDF generator**:
   ```bash
   python scripts/generate_pdf_report.py /tmp/geo-audit-data.json "GEO-REPORT-[brand_name].pdf"
   ```
