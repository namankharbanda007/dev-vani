# Smartmurti AI WebSocket Server (Deno)

This directory contains the WebSocket server implementation for Smartmurti AI, built with Deno.
...
1. **Deploy to Deno Deploy**:
   - Create an account on [Deno Dash Deploy](https://dash.deno.com/account/overview)
      - **Note**: Deno Dash is the legacy version of Deno Deploy. Smartmurti is currently using Deno Dash. You can use Deno Deploy if you prefer but Deno deploy routes websockets via AWS which flips your websocket's RSV bits and can result in unknown opcodes for your ESP32.
